...
void MeshProcessing::OnOpenFile() {
	QStringList file_paths = QFileDialog::getOpenFileNames(this, QString::fromLocal8Bit("??ļ?"), "./objs/", tr("OBJ files(*.obj);; OFF files(*.off);; STL files(*.stl)"));
...
void MeshProcessing::OnReadColorTable() {
	if (this->mesh_processing_data_model_->mesh_vec_.size() != 1) {
		QMessageBox::critical(this, QString::fromLocal8Bit("???), QString::fromLocal8Bit("???Ŀǰ?????????????1??"));
		return;
	}

	this->color_table_handler_ = new ColorTableHandler;
	QString file_path = QFileDialog::getOpenFileName(this, QString::fromLocal8Bit("??ļ?"), "./", tr("txt Files(*.txt)"));
	...
	if (this->color_table_handler_->read() == false) {
		QMessageBox::critical(this, QString::fromLocal8Bit("???), QString::fromLocal8Bit("????ɫ??ļ?????ȷ??"));
		return;
	}
	...
	QMessageBox::information(this, QString::fromLocal8Bit("?ʾ"), QString::fromLocal8Bit("?ɫ???ȡ??ϣ??ѡ??ɫģʽ??"));
}

void MeshProcessing::OnWriteColorTable() {
	if (this->color_table_handler_ == nullptr) {
		QMessageBox::critical(this, QString::fromLocal8Bit("???), QString::fromLocal8Bit("??ǰû??ɫ???"));
		return;
	}

	QString file_path = QFileDialog::getSaveFileName(this, QString::fromLocal8Bit("????ļ?"), "", tr("txt Files(*.txt)"));
	if (file_path.size() == 0) return;

	this->color_table_handler_->setColorTableName(file_path);
	this->color_table_handler_->write();

	QMessageBox::information(this, QString::fromLocal8Bit("?ʾ"), QString::fromLocal8Bit("?ɫ???????ϣ?"));
}
...
void MeshProcessing::OnDisplayNormal() {
	if (this->mesh_processing_data_model_->pick_mode_ != MeshProcessingDataModel::FACE ||
		...
		QMessageBox::warning(this, QString::fromLocal8Bit("????), QString::fromLocal8Bit("???ʰȡģʽ?ѡȡһ???Ƭ??"));
		return;
	}
...
void MeshProcessing::OnICPRegistration() {
	...
	if (active_ids.size() != 2) {
		QMessageBox::warning(this, QString::fromLocal8Bit("????), QString::fromLocal8Bit("???Ŀǰѡ???????????2??"));
		return;
	}
